...
to setup-world
  ...
  set grass count patches with [pcolor = green] ;; Compter l'herbe
end
...
to go-wolf
  ask wolf [
  move ;; déplacement
  set energy energy - 1 ;; perte d'énergie
  catch-sheep ;; mange un mouton s'il est proche
  death ;; mort
  reproduce-wolves ;; reproduction
  ]
end

;; Chasse des moutons
to catch-sheep
  let prey one-of sheep in-radius 1 ;; voisinage pour la détection de moutons
  if prey != nobody [
    ask prey [die]
...
;; Déplacement aléatoire vers cellule libre
to move
...
"wolf" 1.0 0 -16710653 true "" "plot count wolf"
"grass" 1.0 0 -14439633 true "" "if grass? [plot grass / 4 ]"
