...
to wait-for-help
  ifelse food > 0 [
    ifelse count turtles-here with [color = blue] > 1 [
      set color orange
      set food food - 1
    ] [
...
to call-for-help
  ask turtles in-radius broadcast-distance with [color = red] [
    set help-obj [ patch-here ] of myself
    set color green
    set wait-timer wait-time
  ]
end
...
to go-help
  ifelse wait-timer > 0 [
    ...
    wiggle
    fd 1
    if patch-here = help-obj [ set color blue ]
  ] [
    set color red
  ]
end
...
"" ""
PENS
"food-in-pile1" 1.0 0 -11221820 true "" "plotxy ticks sum [food] of patches"
