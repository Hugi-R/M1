...
//#define nbFoisProd   2 //10
//#define nbFoisConso  2 //10
...
pthread_mutex_t mutexResCritique = PTHREAD_MUTEX_INITIALIZER;
...
pthread_cond_t condDepot[2] = {PTHREAD_COND_INITIALIZER, PTHREAD_COND_INITIALIZER};
...
void initialiserVarPartagees (void) {
  ...
  resCritiques.nbCasesVide = nbCases;
  for (i = 0; i < nbCases; i++) {
...
void depot (const TypeMessage *leMessage) {
  ...
  resCritiques.nbCasesVide--;
  resCritiques.dernierType = leMessage->type;
#ifdef TRACE_BUF
  afficherBuffer();
#endif
}
...
void deposer (TypeMessage leMessage, int rangProd) {
  lock(&mutexResCritique);
  while(resCritiques.nbCasesVide == 0 ||
    resCritiques.dernierType == leMessage.type){
    pthread_cond_wait(&condDepot[leMessage.type], &mutexResCritique);
  }
  depot(&leMessage);
  printf("\tProd %d : Message a ete depose = [T%d] %s (de %d)\n",
         rangProd, leMessage.type, leMessage.info, leMessage.rangProd);
  //priorité aux depot car pas moyen de test condition vide
  if(resCritiques.nbCasesVide > 0)
    pthread_cond_signal(&condDepot[1 - resCritiques.dernierType]);
  else
    pthread_cond_signal(&condRetrait);
  unlock(&mutexResCritique);
}
...
void retirer (TypeMessage *unMessage, int rangConso) {
  lock(&mutexResCritique);
  while(resCritiques.nbCasesVide == nbCases){
    pthread_cond_wait(&condRetrait, &mutexResCritique);
  }
  retrait(unMessage);
